                                    ...
                                    children: [
                                      ...
                                      IconButton(
                                          onPressed: () async {
                                            ...
                                            sp.setString('creationDate', doc['date']);
                                                Navigator.push(
                                                context,
                                                PageTransition(
...
class GetReportCardPatientExpert extends StatelessWidget {
  //const GetReportCardPatientExpert({Key? key}) : super(key: key);
  final String uid;
  GetReportCardPatientExpert(this.uid);
  final userReports = FirebaseFirestore.instance.collection("reports");

  @override
  Widget build(BuildContext context) {
    return FutureBuilder<QuerySnapshot>(
        future: userReports.where("reportSavedBy", arrayContains: uid).get(),
      builder:
          (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
        if(snapshot.hasError) {
          return Text("Error fetching reports");
